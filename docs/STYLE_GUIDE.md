...
| `.ingest-event-tag` | Inline badge showing score, reason, or status; `--font-mono` 0.65rem; `--radius-sm`. Full-scrape rows emit only the score tag — `FULL` is the default and not shown. Snippet-fallback rows append `.tier-mid` to the tag for amber quality warning (`SNIPPET`). |
