...
def _clear_pdf_jobs():
    ...
    with pdf_import_module._pdf_jobs_lock:
        pdf_import_module._pdf_jobs.clear()
    yield
    with pdf_import_module._pdf_jobs_lock:
        pdf_import_module._pdf_jobs.clear()
...
class TestImportPdfSync:
    ...
    def test_text_at_threshold_is_sync(
        ...
        text = "x" * pdf_import_module._PDF_ASYNC_THRESHOLD
        resp = self._post(client, text)
        assert resp.status_code == 200
        data = resp.get_json()
        assert "async" not in data
...
class TestImportPdfAsync:
    ...
    def test_large_pdf_returns_202(
        ...
        text = "x" * (pdf_import_module._PDF_ASYNC_THRESHOLD + 1)
    ...
    def test_large_pdf_job_registered_as_pending(
        ...
        with pdf_import_module._pdf_jobs_lock:
            assert job_id in pdf_import_module._pdf_jobs
            assert pdf_import_module._pdf_jobs[job_id]["status"] == "pending"
    ...
    def test_large_pdf_submits_to_executor(
        ...
        assert call_args[0][0] is pdf_import_module._run_pdf_import_job
...
class TestImportPdfStatus:
    ...
    def test_pending_job_returns_status_pending(
        ...
        with pdf_import_module._pdf_jobs_lock:
            pdf_import_module._pdf_jobs[job_id] = {
                "id": job_id,
                "status": "pending",
                "result": None,
                "error": None,
...
class TestPrunePdfJobs:
    ...
    def test_prune_removes_old_complete_job(self):
        ...
        with pdf_import_module._pdf_jobs_lock:
            pdf_import_module._pdf_jobs[job_id] = {
                ...
                "result": {},
                "error": None,
                "created_at": datetime.now(timezone.utc).timestamp() - pdf_import_module._PDF_JOB_TTL_SECONDS - 1,
            }
        pdf_import_module._prune_pdf_jobs()
        with pdf_import_module._pdf_jobs_lock:
            assert job_id not in pdf_import_module._pdf_jobs
    ...
    def test_prune_keeps_recent_running_job(self):
        ...
        pdf_import_module._prune_pdf_jobs()
        with pdf_import_module._pdf_jobs_lock:
            assert pdf_import_module._pdf_jobs[job_id]["status"] == "running"
...
class TestRunPdfImportJob:
    ...
    def test_worker_sets_complete_on_success(self, tmp_profile_path):
        ...
        with (
            ...
            pdf_import_module._run_pdf_import_job(
                job_id, "resume text", "fresh", {}, tmp_profile_path
            )

        with pdf_import_module._pdf_jobs_lock:
            job = pdf_import_module._pdf_jobs[job_id]
    ...
    def test_worker_transitions_through_running_status(self, tmp_profile_path):
        ...
        def fake_generate(prompt, chain, failures):
            with pdf_import_module._pdf_jobs_lock:
                observed_statuses.append(pdf_import_module._pdf_jobs[job_id]["status"])
...
class TestConcurrencyLimit:
    ...
    def test_returns_429_when_at_limit(
        ...
        with pdf_import_module._pdf_jobs_lock:
            for i in range(pdf_import_module._MAX_CONCURRENT_PDF_JOBS):
                pdf_import_module._pdf_jobs[f"running-{i}"] = {
                    "id": f"running-{i}",
                    "status": "running",
                    "result": None,
                    "error": None,
    ...
    def test_stuck_running_job_gets_timed_out(self):
        ...
        with pdf_import_module._pdf_jobs_lock:
            pdf_import_module._pdf_jobs[job_id] = {
                ...
                "started_at": datetime.now(timezone.utc).timestamp()
                - pdf_import_module._PDF_JOB_TIMEOUT_SECONDS
                - 1,
            }
        pdf_import_module._prune_pdf_jobs()
        with pdf_import_module._pdf_jobs_lock:
...
class TestPruneRateLimit:
    ...
    def test_prune_skips_when_called_within_interval(self):
        ...
        pdf_import_module._prune_pdf_jobs()
        with pdf_import_module._pdf_jobs_lock:
            assert job_id in pdf_import_module._pdf_jobs  # not pruned because rate-limited
